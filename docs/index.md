# Arista Western Canada — Client Newsletter · May 2026

Welcome to the inaugural edition of the Arista Western Canada client newsletter. Each month this publication covers software updates, security advisories, product news, and regional updates for customers across Alberta, Manitoba, and Saskatchewan.

---

## __*Software Updates*__
![Image Placememt](img/software_upgrades_condensed.png)

*Stay informed on the latest software updates across all Arista products and services.*

|  Software    | Version      |  Release Date |
| :-----------: | :-----------: | :-----------: |
| __EOS__           | 4.36.0F <br> 4.35.3.1F <br> 4.35.3F <br> 4.23.10M | April 9th, 2026 <br> April 8th, 2026 <br> March 20th, 2026 <br> March 2nd, 2026 |
| __CVP__           | Portal 2026.1.0 <br> Appliance 7.1.0 <br> Sensor 1.3.0 | March 30th, 2026 <br> September 2nd, 2025 <br> December 5th, 2025 |
| __DMF__           | 8.6.3 | March 18th, 2026 |
| __CV-CUE__         | 21.0.0 | January 16th, 2026 |
| __Arista NDR__     | 5.3.5 | July 16th, 2025 |
| __TerminAttr__     | 1.42.1 | February 4th, 2026 |
| __VeloCloud SD-WAN__ <br>Orchestrator/Gateway/Edge | 6.4.1 | December 19th, 2025 |

[View All Latest Software Updates](https://www.arista.com/en/support/software-download){: .md-button .md-button--primary target="_blank" }




---

## Security Advisories & Field Notices

### Security Advisories

- **runC — [Security Advisory 0135](https://www.arista.com/en/support/advisories-notices/security-advisory/23784-security-advisory-0135)**
  *(April 7, 2026)*
  Affects CVP Portal and CloudVision as a Service. Remediated in CVP 2026.1.0. CVaaS customers receive automatic updates.

### Field Notices

- **CV-CUE Release Process — [Field Notice 0125](https://www.arista.com/en/support/advisories-notices/field-notice/23698-field-notice-0125)**
  *(March 11, 2026)*
  Review recommended upgrade sequencing before upgrading to CV-CUE 21.0.0 from older versions.

[View all advisories & notices](https://www.arista.com/en/support/advisories-notices){ .md-button }

---

## __* Product Updates*__

![Image Placememt](img/Product_image.png)

*Stay up to date on all new Arista Product Releases, as well as End of Sale/End of Support Notices.*

### **New Product Releases** * **Q1 2026** — [Ask AVA - CloudVision as a Service (beta feature)](https://www.arista.io/help/articles/overview-core-tools-ask-ava){: target="_blank" }

###  **End of Sale / End of Software Support**
* **April 10th, 2026** — [CloudVision Portal 2024.3 Release Train](https://www.arista.com/en/support/advisories-notices/end-of-support/23791-end-of-software-support-for-cloudvision-portal-2024-3-release-train){: target="_blank" } 
* **April 10th, 2026** — [DCS-7280CR3K-36S Series Switches](https://www.arista.com/en/support/advisories-notices/end-of-sale/23790-end-of-sale-of-the-arista-dcs-7280cr3k-36s-series){: target="_blank" } 
* **April 10th, 2026** — [DCS-7280CR3K-48YC8 Series Switches](https://www.arista.com/en/support/advisories-notices/end-of-sale/23789-end-of-sale-of-the-arista-dcs-7280sr3k-48yc8-series){: target="_blank" } 
* **April 10th, 2026** — [DCS-7280CR3K-32P4, DCS-7280CR3K-32D4, DCS-7280CR3MK-32P4 Series Switches](https://www.arista.com/en/support/advisories-notices/end-of-sale/23788-end-of-sale-of-the-arista-dcs-7280cr3k-32p4-dcs-7280cr3k-32d4-and-dcs-7280cr3mk-32p4-series){: target="_blank" }
* **March 13th, 2026** — [EOS-4.30 Release Train](https://www.arista.com/en/support/advisories-notices/end-of-support/23728-end-of-software-support-for-eos-4-30){: target="_blank" } 

<br>

[View All Latest End of Sale & Support Notices](https://www.arista.com/en/support/advisories-notices/endofsale){: .md-button .md-button--primary target="_blank" }





---

## Vertical Focus

### Oil & Gas — Alberta

Single EOS platform across HQ and remote field sites, with streaming telemetry to CloudVision for remote visibility. EOS 4.36.0F includes streaming telemetry improvements relevant to high-latency remote site deployments.

- Ruggedized switching variants available for harsh environments
- CVP 2024.3 EoSS is urgent — upgrade to 2026.1.0 resolves both EoSS and SA-0135

### Municipalities — AB · MB · SK

Campus in a Box modular bundles for municipal offices and multi-site networks. Available through Canoe Procurement Group for eligible public sector organizations.

Priority targets: Lethbridge, Grande Prairie, Prince Albert, Swift Current, Steinbach.

### K-12 Education — MB · SK · Alberta

CloudVision zero-touch provisioning reduces per-site deployment time for school districts managing dozens of campuses.

Priority targets: Pembina Trails SD, River East Transcona (MB); Lethbridge SD #51, Wolf Creek (AB); Saskatoon Public Schools (SK).

---

## Events & Webinars

**Upperbound AI — Edmonton, AB** *(May 2026)*
Arista will be present. Contact your account manager for meeting scheduling.

**Campus Modernization Briefings** *(Q2 2026 — Calgary · Winnipeg · Saskatoon)*
Technical briefings co-hosted with Compugen. Contact your Arista AM or Compugen representative for dates.

[View Arista webinars](https://www.arista.com/en/company/news/webinars){ .md-button }

---

*Reach out to your Arista account manager or SE with questions about any content above.*

*© 2026 Arista Networks · Western Canada Territory · AB / MB / SK*
